...
window.addEventListener('orientationchange', doOnOrientationChange);
  function doOnOrientationChange(){
    ...
    {  
  		$("#map").css({width: '100%', height: window.innerWidth * .40 + ""});
    }
    else//portrait
    {
...
function getMap(layerIndex, deferred) // making deferred a paramter is messy organization-wise, but it enables first-load optimization
{
	...
	if(deferred != null) // if NOT a layer change refresh
		...
		.fail(function( jqxhr, textStatus, error ) {
			console.log("JSON has failed to load! Please contact Carola Kaiser at ckaiser@cct.lsu.edu. Tell her about error code " + textStatus + ", " + error);
		});
